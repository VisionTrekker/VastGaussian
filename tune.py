...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, pretrained_ply):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        # Every 1000 its we increase the levels of SH up to a maximum degree，finetune 过程中不增加球谐系数
        # if iteration % 1000 == 0:
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                finetune_ite = searchForMaxIteration(os.path.join(dataset.model_path, "point_cloud")) + iteration
                print("\n[ITER {}] Saving Gaussians".format(finetune_ite))
                scene.save(finetune_ite)
...
if __name__ == "__main__":
    ...
    print("Finetuning " + args.model_path)
